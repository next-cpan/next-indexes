...
sub set_default_user_repo {
    ...
    if ( $self->is_main_module ) {
        ...
        if ( $self->is_pull_request_init ) {
            $self->pull_request->u($user);
            $self->pull_request->repo($repo);
        }
        if ( $self->is_git_data_init ) {
            $self->git_data->u($user);
            $self->git_data->repo($repo);
        }
    }
...
sub query {
    ...
    if ( $self->api_throttle ) {
        sleep 2 if ( ( $self->rate_limit_remaining || 0 ) < ( $self->rate_limit || 60 ) / 2 );
    }
        ...
        if ( not $res->is_success and ref $data eq 'HASH' and exists $data->{message} ) {
            ...
            if ( exists $data->{errors} ) {
                $message .= ': ' . join(
                    ' - ',
                    map    { $_->{message} }
                      grep { exists $_->{message} } @{ $data->{errors} }
                );
            }
            croak $message;
        }
